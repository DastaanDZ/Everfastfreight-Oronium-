...
const index = ({branches}) => {
  ...
  return (
    ...
    {branches.data.map((items,index) =>{
        ...
        <div className={styles.bullet}><p>{items.id}</p></div>
        <div className={styles.title}><p>{items.attributes.Branches}</p></div>
        <div className={styles.manager}><p>{items.attributes.Manager}</p></div>
